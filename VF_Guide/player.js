...
async function serverRewards(itemId, particleName) {
  ...
  try {
    const [emoteRes, particleRes] = await Promise.all([
      fetch('https://working-emperor-reg-heat.trycloudflare.com/webhook/grant-inventory-item', {
        method: 'POST',
      ...
      fetch('https://working-emperor-reg-heat.trycloudflare.com/webhook/play-particle', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      }).then(r => {
